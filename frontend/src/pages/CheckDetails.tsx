...
function PayloadViewer({ pingId }: { pingId: string }) {
    ...
    const fetchPayload = async () => {
      ...
      try {
        ...
        if (ct.includes('text/') || ct.includes('application/json')) {
          ...
          if (isMounted)
            setContent('Binary or unsupported payload format.')
        }
      }
      catch (err) {
        if (isMounted)
          setError((err as Error).message || 'Error loading payload')
      }
      finally {
        if (isMounted)
          setLoading(false)
  ...
  if (loading)
    return <Text color="gray" size="2">Loading payload...</Text>
  if (error)
    return <Text color="ruby" size="2">{error}</Text>
    ...
    <Container size="4" py="6">
      <Flex mb="8" justify="between" align="center">
        <Flex gap="4" align="center">
          <Button variant="ghost" onClick={() => navigate('/')} color="gray" style={{ cursor: 'pointer' }}>
            <ArrowLeft size={16} />
            {' '}
            Back
          </Button>
        ...
        <Flex gap="3">
          ...
          <Dialog.Root>
            <Dialog.Trigger>
              <Button variant="soft" color="ruby" style={{ cursor: 'pointer' }}>
                <Trash2 size={16} />
                {' '}
                Delete
              </Button>
            ...
            <Flex direction="column" gap="4" mb="5">
              ...
              <Grid columns="2" gap="4">
                <label>
                  ...
                  <TextField.Root size="3" type="number" min="1" value={intervalMin} onChange={e => setIntervalMin(Number.parseInt(e.target.value) || 1)} />
                </label>
                <label>
                  <Text as="div" size="2" mb="2" weight="medium">Grace Period (minutes)</Text>
                  <TextField.Root size="3" type="number" min="1" value={graceMin} onChange={e => setGraceMin(Number.parseInt(e.target.value) || 1)} />
                </label>
              </Grid>
            </Flex>
      ...
      <Dialog.Root open={!!selectedPingForPayload} onOpenChange={open => !open && setSelectedPingForPayload(null)}>
        <Dialog.Content style={{ maxWidth: 600 }}>
          ...
          <Dialog.Description size="2" mb="4">
            Payload details for ping at
            {' '}
            {selectedPingForPayload && format(new Date(selectedPingForPayload.createdAt), 'PPpp')}
          </Dialog.Description>
